...
- **The budget is a count for the read (Rina, 23 Sep).** The person's budget is a ceiling (§5.2), but the read is told to return the top of the range and never less than the bottom, and that a longer cut is filled with coverage — each moment shot more ways — not more topics. A read that still comes back short is filled on B2 from the brief-matched library under **TO REACH THE BUDGET**, the first ones added with the rest. Case-study-log 2.26.
- **A read counts only when it comes back (Rina, 23 Sep).** A running read holds a place against the limit, so reads can't be fired at once to dodge it. It's counted only when it succeeds. A read that fails, or whose server is killed mid-way, costs nothing: the hold is released, or simply expires after six minutes.
- **While it reads (Rina, 23 Sep).** A minute is long to watch a still screen, so the read streams its progress, and the screen shows only what really happened:
  - a stage line: *Sending your brief…*, *Reading it through…*, *Planning the shots — 12 of 64*, *Finding a few more to reach your budget…* when the backstop runs, and *Putting it together…*;
  - under it, the subject of the shot just written;
  - a 3px meter filling against the budget, and the elapsed time;
  - a note to keep the screen open.

  Leaving mid-read stops the model, and nothing is charged. Screen readers hear each stage once, not every shot.
- **Timing (23 Sep).** A 5–10 minute read writes about 64 shots. At the default effort it ran past the server's 120s limit and failed, so the read runs at effort `medium`, with one-sentence reason lines, inside a 300s window: about a minute for 63 shots. `READ_EFFORT` in `read.ts`.
...
- **Suggests locations when there are none,** so its shots arrive placed, not in `UNPLACED`. B2 marks them `NAME · NEW`, and adding a shot creates its location (§10, 23).
...
**Edit mirrors add**, field for field, so there is nothing new to learn.

**Duplicate and delete sit behind `⋯`, never on the page (changed 25 Sep, Rina).** They used to be two rows on shot detail, under the spec and the note, where they were too easy to hit by accident. Now shot detail's header carries `03 / 12` and a `⋯` (S3), and `⋯` opens a sheet (S3a) titled `SHOT 03 · HANDS ON THE ROPE`:

- **`EDIT SHOT`** — "Size, lens, movement, sound, note." Opens edit (S6).
- **`DUPLICATE`** — "Opens a copy to change. Nothing is added until you save it." Opens add shot filled from the shot (S4c). The copy's spec, location, beat, note and `[★]` client are all carried over, and a band says `COPY OF 03 · HANDS ON THE ROPE` / `NOT ADDED YET`. The header already reads `New shot · 04`. The foot says "Goes in after 03, and the shots below move down one. Cancel and nothing is added." The button is **`ADD AS 04`**, and only that adds it, straight after the original. Change its location first and it goes at the end of the new one, like any added shot. The duplicate warning is silent for a copy that matches its own original — that's the point of it.
- **`DELETE SHOT`** — "Asks you first." The confirmation (S3b) replaces the sheet's content. It shows `DELETE · SHOT 03` in `--warn`, then "Delete this shot?", then what goes and what moves: *"Hands on the rope goes, with its note and 3 refs. The shots below move up — 04 becomes 03. This can't be undone."* The buttons are `YES, DELETE` and `CANCEL`. A number is a position, not an identity (§5.10).

**Picking shots: long-press, or `SELECT` (added 25 Sep, Rina).** Long-pressing a shot row enters select mode with that shot picked (SL3). `SELECT` on a band stays as the way in that doesn't depend on a gesture. While picking:
- **Top of the list:** a strip on `--field` says `N SELECTED`, with `CANCEL`.
- **Rows:** each shows a tick box, then its number (picked rows on `--field`), and a tap adds or removes it.
- **End of the list:** "Long-press any shot to start picking, or SELECT on a band. Tap more to add them."
- **The bar**, one shot picked: `DUPLICATE · MOVE… · DELETE` (SL3). `DUPLICATE` opens S4c; `MOVE…` opens the move sheet (SL2); `DELETE` opens S3b.
- **The bar**, several picked: `DELETE N` and `MOVE N TO…` (SL4).
- **Deleting several asks first (SL5):** `DELETE · 3 SHOTS`, "Delete 3 shots?". A `--warn` box names any that are already shot (*"! 1 IS ALREADY SHOT — 02 Hero walking, tracking — its mark goes with it, and it leaves the exposed count."*). Another names any `[★]` shots, and who they're owed to. Then "The shots after them move up to fill the gaps. This can't be undone." and `YES, DELETE 3` · `CANCEL`. A single delete names these too, in the same boxes.
